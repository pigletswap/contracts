...
contract PigletSwap is BEP20 ('Piglet Swap Token', 'PIGLET'), MinterRole {


 
    uint256 private constant _initialSupply = 10*10**18;
    ...
    function transfer(address _to, uint256 _value) external   returns (bool) {
        ...
        uint256 burnAmount = _value.mul(burnRate).div(100);
        uint256 sendAmount = _value.sub(burnAmount);


        if (_to == burnAddress || _to == address(0)) {
                   super._transfer(msg.sender, burnAddress, _value);

        } else {

                super._transfer(msg.sender, _to, sendAmount);
                super._transfer(msg.sender, burnAddress, burnAmount);
   ...
   function updateBurnRate(uint16 _burnRate) public onlyOwner {
        require(_burnRate <= 20, "PIGLET::updateBurnRate: Burn rate must not exceed the maximum rate.");
